...
py -3 -m pip install kokoro-onnx sounddevice numpy pywebview pystray pillow --quiet
Write-Host "      Done."
...
Set-Content -Path "$kokoro\panel.html" -Encoding UTF8 -Value @'
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
Set-Content -Path "$kokoro\logo.svg" -Encoding UTF8 -Value @'
...
Set-Content -Path "$kokoro\favicon.svg" -Encoding UTF8 -Value @'
<?xml version="1.0" encoding="UTF-8"?>
<svg id="Layer_1" xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 792.2 686.13">
  <!-- Generator: Adobe Illustrator 30.6.0, SVG Export Plug-In . SVG Version: 2.1.4 Build 109)  -->
  <defs>
...
'@
[IO.File]::WriteAllBytes("$kokoro\panel.ico", [Convert]::FromBase64String(($panelIcoBase64 -replace '\s','')))
...
YAJgMHwMJgAGw8dgAmAwfAwmAAbDx2ACYDB8DCYABkP4F/8PjufFgqikIr8AAAAASUVORK5CYII=
'@
[IO.File]::WriteAllBytes("$kokoro\favicon.ico", [Convert]::FromBase64String(($faviconIcoBase64 -replace '\s','')))

Set-Content -Path "$kokoro\panel_app.py" -Encoding UTF8 -Value @'
# -*- coding: utf-8 -*-
"""
...
# Launchers (ASCII: a BOM would break cmd.exe)
Set-Content -Path "$claude\Open-Panel.bat" -Encoding ASCII -Value @'
@echo off
REM Opens the Omnicapable Voice control panel as a native mini-app window.
REM Falls back to a chromeless Edge app window if pywebview is unavailable.
setlocal
set URL=http://127.0.0.1:59010
set APP=%USERPROFILE%\.claude\kokoro\panel_app.py
...
if exist "%EDGE%" (
  start "" "%EDGE%" --app=%URL% --window-size=380,700 --window-position=120,80
) else (
  start "" %URL%
)
'@
Set-Content -Path "$claude\Open-Panel-Pinned.bat" -Encoding ASCII -Value @'
@echo off
REM Opens the control panel PINNED (floats above all other windows).
call "%~dp0Open-Panel.bat" --top
'@
Set-Content -Path "$claude\Open-Panel.vbs" -Encoding ASCII -Value @'
Set WshShell = CreateObject("WScript.Shell")
Set Fso = CreateObject("Scripting.FileSystemObject")
ScriptDir = Fso.GetParentFolderName(WScript.ScriptFullName)
Cmd = Chr(34) & Fso.BuildPath(ScriptDir, "Open-Panel.bat") & Chr(34)
...
# Shortcuts: Desktop (most discoverable) + Start Menu. Pin either to the taskbar
...
try {
    ...
    foreach ($dir in @([Environment]::GetFolderPath('Desktop'),
        ...
        if (-not $dir) { continue }
        $sc = $ws.CreateShortcut((Join-Path $dir 'Omnicapable Voice.lnk'))
        $sc.TargetPath       = "$env:WINDIR\System32\wscript.exe"
        $sc.Arguments        = '"' + "$claude\Open-Panel.vbs" + '"'
        $sc.WorkingDirectory = $claude
        $sc.Description      = 'Open the Omnicapable Voice control panel'
        $sc.WindowStyle      = 7          # start minimised: no console flash
        if (Test-Path "$kokoro\panel.ico") { $sc.IconLocation = "$kokoro\panel.ico" }
        $sc.Save()
    }
} catch { }
...
Remove-Item "`$startup\omnicapable_voice_tray.vbs" -ErrorAction SilentlyContinue
Write-Host "      Done."
...
Write-Host " Remove with: pip uninstall kokoro-onnx sounddevice pystray pillow"
Write-Host "============================================"
Write-Host ""
"@
...
# --- Windows notification-area tray helper ----------------------------------
Set-Content -Path "$kokoro\tts_tray.py" -Encoding UTF8 -Value @'
# -*- coding: utf-8 -*-
"""
...
if __name__ == "__main__":
    ...
    main()
'@
Set-Content -Path "$startupFolder\omnicapable_voice_tray.vbs" -Value @"
Set WshShell = CreateObject("WScript.Shell")
WshShell.Run "py -3 ""$kokoro\tts_tray.py""", 0, False
"@
Start-Process py -ArgumentList "-3 `"$kokoro\tts_tray.py`"" -WindowStyle Hidden
Write-Host "      Omnicapable Voice tray icon installed (auto-start at login)."
# --- Ctrl+Alt+X/R/Space global hotkeys: safe RegisterHotKey daemon (no low-level keyboard hook) ---
...
Write-Host " Open panel:   press Ctrl+Alt+Space"
...
Write-Host " Open:    Ctrl+Alt+Space"
Write-Host ""
